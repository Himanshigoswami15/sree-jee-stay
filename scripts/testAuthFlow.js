...
async function runTests() {
  console.log('--- Starting Password Auth Flow Verification Tests ---');

  // Test 1: Verify default password (1234)
  console.log('\n[Test 1] Verifying initial password "1234"...');
  const t1 = await post('/api/auth/verify', { tenantId: 'demo', password: '1234' });
  console.log('Result:', t1);
  if (t1.status !== 200 || !t1.data.success) throw new Error('Test 1 failed');

  // Test 2: Verify wrong password
  console.log('\n[Test 2] Verifying wrong password "wrongpass"...');
  const t2 = await post('/api/auth/verify', { tenantId: 'demo', password: 'wrongpass' });
  ...
  const t3 = await post('/api/auth/change-password', { tenantId: 'demo', oldPassword: 'wrong', newPassword: '5678' });
  ...
  const t4 = await post('/api/auth/change-password', { tenantId: 'demo', oldPassword: '1234', newPassword: '5678' });
  ...
  const t5 = await post('/api/auth/verify', { tenantId: 'demo', password: '1234' });
  ...
  const t6 = await post('/api/auth/verify', { tenantId: 'demo', password: '5678' });
  console.log('Result:', t6);
  if (t6.status !== 200 || !t6.data.success) throw new Error('Test 6 failed');

  // Reset back to "1234" for smooth demo state
  console.log('\n[Reset] Resetting password back to default "1234"...');
  await post('/api/auth/change-password', { tenantId: 'demo', oldPassword: '5678', newPassword: '1234' });
